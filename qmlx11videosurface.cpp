...
QmlX11VideoSurface::QmlX11VideoSurface(QObject *parent)
...
QmlX11VideoSurface::~QmlX11VideoSurface()
{
    if (m_gc)
...
WId QmlX11VideoSurface::winId() const
{
    return m_winId;
}

void QmlX11VideoSurface::setWinId(WId id)
{
    //qDebug() << "setWinID:" << id;
...
void QmlX11VideoSurface::setAttribute(const char *attribute, int value, int minimum, int maximum)
{
    if (m_portId != 0) {
        Display *display = QX11Info::display();
...
int QmlX11VideoSurface::redistribute(
...
QList<QVideoFrame::PixelFormat> QmlX11VideoSurface::supportedPixelFormats(
...
bool QmlX11VideoSurface::start(const QVideoSurfaceFormat &format)
{
    if (m_image)
        XFree(m_image);
...
bool QmlX11VideoSurface::present(const QVideoFrame &frame)
{
    if (!m_image) {
...
bool QmlX11VideoSurface::findPort()
{
...
void QmlX11VideoSurface::querySupportedFormats()
{
    int count = 0;
